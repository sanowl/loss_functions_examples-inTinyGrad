...
def huber_loss(y_true, y_pred, delta=1.0):
    error = y_true - y_pred
    with no_grad():
        # Convert memoryview to numpy array before comparison
        error_abs = error.abs().data  # memoryview
        error_abs_np = np.array(error_abs)  # numpy array
        mask_np = (error_abs_np <= delta).astype(np.float32)  # float32 mask
        mask_tensor = Tensor(mask_np, requires_grad=False)  # Tensor mask
    small_error_loss = 0.5 * error ** 2
    big_error_loss = delta * (error.abs() - 0.5 * delta)
    loss = small_error_loss * mask_tensor + big_error_loss * (1.0 - mask_tensor)
    return loss.mean()
...
def example_mnist_cross_entropy():
    ...
    X_train = Tensor(X_train.reshape(-1, 28*28).astype(np.float32) / 255.0)
    Y_train_labels = Y_train.astype(np.int32)
    Y_train_one_hot = Tensor(np.eye(10)[Y_train_labels].astype(np.float32))
    ...
    for epoch in range(epochs):
        ...
        X_train_shuffled = Tensor(np.array(X_train.data()).astype(np.float32)[permutation], requires_grad=False)
